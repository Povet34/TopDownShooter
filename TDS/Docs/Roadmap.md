...
> - **검증**: 입력 주입으로 W→ 전진(스폰 위치서 vel 13.9m/s·50m+), `steerInput=+1`(D)→heading 0→94.7(우회전 정상). EditMode 259 / PlayMode 68 green. (손맛·회전속도는 `maxSpeed`/`accleerationSpeed`/`turnSensetivity`로 튜닝.)
