...
1. **맵 복잡도 ↑ (군집/구조물)** — 균일 산포 → **장애물 군집(클러스터) + 내부 벽/구조물/초크포인트**로 덜 휑하게. 시드 결정적 유지. (구현 가능 — 다음 슬라이스 후보.)
...
3. **🆕 터레인 굴곡 (맵 자체 고저차)** — 오브젝트로 만든 고저차 말고, **터레인처럼 지면 자체가 굴곡**(롤링 힐). 큰 변경:
   - 평면 바닥 → **Unity Terrain 또는 변형 메시(heightmap, 시드 결정적 noise)**.
   - **⚠️ 충돌**: 방금 넣은 평면-맵 **Y-lock**(위로 오른 수직분 차단)이 **오르막을 막아버림** → 굴곡 지형이면 Y-lock을 "지형 경사 한도(slopeLimit) 내 상승은 허용"으로 재설계 필요. 적 navmesh도 경사 위에 베이크.
   - **구현 전 사용자와 합의 필요**(Y-lock 처리·터레인 vs 메시·navmesh 경사).
4. **주변만 렌더링** — 큰/복잡 맵 성능용. **현재 탑다운 카메라는 이미 프러스텀 컬링**돼 보이는 영역만 그림(1024서 draw call 50). 즉 "내 주변만 렌더"는 상당 부분 이미 동작 중. 더 공격적으로(가까운 청크만 생성/활성, 거리 컬링/포그)가 필요하면 별도. (대형 스티칭 맵 때 본격.)
