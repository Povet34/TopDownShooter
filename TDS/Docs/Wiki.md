...
**구현**: 순수 `ViewCone`(콘+거리, EditMode 9) + `FieldOfView`(콘+사거리+눈높이 레이캐스트 차폐+nearRadius+Reveal, 적 renderer on/off, PlayMode 4) + `Player_FieldOfView`(조준 방향 구동·발사 시 Reveal). 비주얼 = `VisionMask`(가시성을 텍셀에 구워 Texture2D 마스크, 3x3 블러, 글로벌 `_VisionMask`/`_VisionMaskCenterSize`) + `Shaders/VisionFog`(지면 위 fog 쿼드, `alpha=(1-mask)*MaxDarkness`로 시야 밖 회색, PlayMode 1). occluder=Default|Environment(낮은 cover는 넘어 봄). in-game·마스크 텍셀(전방 1.0/후방 0.0)·명도 검증.
- **해상도**: 맵 전체가 아니라 **플레이어 주변 영역(regionSize)만** 덮어 같은 텍셀로 조밀하게(기본 res 160 / region 48 = 0.30u/텍셀). 계산은 `framesPerMask` 프레임에 분산(스파이크 방지), 중심은 텍셀 그리드 스냅(shimmer 방지). resolution·regionSize·framesPerMask·fogColor·maxDarkness 등 **전부 Inspector 프로퍼티**로 튜닝.
