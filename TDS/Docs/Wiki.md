...
- **대형 맵 1024×1024 (2026-06-24)**: `MapConfig_Default` = grid 256×256 × cellSize 4 = **1024 월드**. 깔끔한 성능을 위해:
  ...
  - **절벽(못 올라가는 지형, 2026-06-25)**: `interiorCliffCount` 내부 메사(1개 = 1~3개 지터 블록, 높이 `cliffHeight` 기본 10, 풋프린트 `cliffMin~MaxFootprint`) + `boundaryAsCliff`로 경계 벽을 절벽 높이/`cliffMaterial`(Mat_Cliff)로 올림. 블록이 높아 navmesh가 자동 제외(가팔라 못 걸음) → 엔티티가 절벽 안으로 못 들고 **우회**(`CliffTests`가 A→B 직선이 우회로 강제되는지로 검증). 걷는 평지는 그대로 평평(Y-lock 무관). 시드 결정적, cullable. `ApplyRockMaterial`이 풋프린트에 맞춰 텍스처 타일링. in-game 1612오브젝트/27블록, 325fps/draw call 58.
  - **바위 장식(2026-06-25)**: `capCliffsWithRocks`(절벽 위를 작은 바위로 덮음 — 탑다운서 윗면 보임) + `rockPropCount`/`rockPropSize`(바닥 산재 바위). `Mat_RockProp`(Rocks012), 비균일 스케일+회전으로 각진 돌. **콜라이더 제거 → 순수 시각**(네브메시/이동 영향 0), cullable. `SpawnRock` 공통(`fullTumble`=절벽 위 완전 회전 / 바닥은 살짝).
  - **실측(에디터 플레이)**: 런타임 ~95~130fps, draw call 50 / batches 49(사막 props 머티리얼 공유로 정적배칭), tris ~95k. 생성+베이크 일회성 ~2.2~2.9s.
- **데이터 기반 콘텐츠** (`Assets/GameData/Map/MapConfig_Default`): 사막 황무지 테마 — 바닥 머티리얼(**`Mat_DesertGround`** = AmbientCG Ground098 PBR: Color+NormalGL+AO, 모래 물결 — 기존 단조로운 단색 대체), 장애물 풀(부서진 차/연료탱크/콘크리트관/사막바위/돌/선인장), 엄폐물(`sea_container`).
- **바닥 텍스쳐 + 노이즈 블렌딩 (2026-06-25)**: 통짜 타일이 단조로워서 커스텀 URP 셰이더 **`TDS/DesertGroundBlend`**(`Mat_DesertGroundBlend`) 도입 — 모래(Ground098)를 풀디테일로 유지하되, **월드 XZ fbm 노이즈**로 ① 바위 패치(Rocks012)를 부드럽게 얹고 ② 거대 스케일 명암 변주를 줘 반복 티를 깬다. 타일링과 독립(월드 좌표 기반)이라 패턴이 자연스러움. `floorTileWorldUnits`(8) → `_BaseMap`/`_SecondMap`/normal 모두 같은 스케일(1024서 128×128). `[MainTexture]`로 `_BaseMap` 태그 + `MapGenerator`가 `SetTextureScale("_BaseMap")` 명시(안 그러면 텍스처가 1024 전체로 늘어나 blur). ~158fps, 영향 없음. (단색 `Mat_DesertGround`는 폴백으로 남김.) 절벽/돌용 `Mat_Cliff`(Rock029)·`Mat_RockProp`(Rocks012)도 준비(절벽 메시 슬라이스에서 사용). 임포트 프리팹은 바닥(y=0) 배치, 정적 MeshCollider는 `convex=false`(navmesh 카빙 + 충돌).
